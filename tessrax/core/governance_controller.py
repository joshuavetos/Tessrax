...
def _ensure_tessrax_package_stub() -> None:
    ...
    tessrax_stub.__spec__ = importlib.machinery.ModuleSpec('tessrax', loader=None, is_package=True)
...
try:
    ...
    if not hasattr(tessrax, '__spec__') or getattr(tessrax.__spec__, 'submodule_search_locations', None) is None:
        _ensure_tessrax_package_stub()
...
class GovernanceController:
    ...
    def __init__(self, *, output_path: Path = AUDIT_OUTPUT_PATH, receipts_path: Path = AUDIT_RECEIPTS_PATH) -> None:
    ...
    def _evaluate_summary(self, summary: Dict[str, Any]) -> List[AuditCheck]:
        ...
        numeric_scores = [score for score in alignment_scores.values() if isinstance(score, (int, float))]
        score_bounds_passed = bool(numeric_scores) and all(0.0 <= float(score) <= 1.0 for score in numeric_scores)
        checks.append(
            AuditCheck(
        ...
        checks.append(
            AuditCheck(
                ...
                details={"contradiction_count": len(contradiction_cases) if isinstance(contradiction_cases, list) else None},
            )
        )
        category_counts = summary.get("category_counts", {})
        well_structured_categories = isinstance(category_counts, dict) and all(
            isinstance(key, str) and isinstance(value, int) and value >= 0 for key, value in category_counts.items()
        )
        checks.append(
            AuditCheck(
                name="category_distribution_valid",
                passed=well_structured_categories,
                details={"category_count": len(category_counts) if isinstance(category_counts, dict) else None},
            )
        )
        return checks
    ...
    def _append_receipt(self, report: Dict[str, Any]) -> None:
        ...
        receipt = {
            ...
            "checks": [check["name"] for check in report.get("checks", []) if check.get("passed")],
        }
...
def _main() -> int:
    ...
    try:
        ...
        traceback.print_exc()
        print(f"[Tessrax] Governance audit encountered an error: {exc}", file=sys.stderr)
        return 1
...
def _self_test() -> bool:
    ...
    controller = GovernanceController(output_path=Path("out/_self_test_governance_audit.json"), receipts_path=Path("out/_self_test_receipts.jsonl"))
    result = controller.run()
