...
def _merkle_root(lines: List[str]) -> str:
    ...
    while len(layer) > 1:
        ...
        for index in range(0, len(layer), 2):
            ...
            next_layer.append(sha256(f"{left}{right}".encode("utf-8")).hexdigest())
        layer = next_layer
    return layer[0]
...
def append_audit_receipt(audit_json: Dict[str, Any]) -> Dict[str, Any]:
    ...
    write_receipt("tessrax.core.ledger.audit_integration.append", "verified", metrics, 0.96)
...
def verify_audit_receipt(receipt_id: str) -> Dict[str, Any]:
    ...
    write_receipt("tessrax.core.ledger.audit_integration.verify", "verified", metrics, 0.95)
