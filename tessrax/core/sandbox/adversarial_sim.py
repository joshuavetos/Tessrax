...
def inject_noise(
    receipts: Sequence[ReceiptRecord],
...
) -> dict[str, Iterable[ReceiptRecord]]:
...
def evaluate_recovery() -> dict[str, float | list[dict[str, str]]]:
    ...
    if tampered_set:
        true_positive = sum(
            1 for identifier in detections if identifier in tampered_set
        )
...
def _self_test() -> bool:
    ...
    assert (
        len(metrics["recovery_log"]) == metrics["detections"]
    ), "Recovery log mismatch"
    return True
