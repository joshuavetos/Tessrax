...
def write_receipt(module: str, status: str, metrics: Dict[str, Any], integrity: float) -> Receipt:
    ...
    signature = sha256(f"{module}|{status}|{metrics_hash}|{timestamp}".encode("utf-8")).hexdigest()
    receipt = Receipt(
        module=module,
        status=status,
        metrics=metrics,
...
def __receipt_to_json(receipt: Receipt) -> str:
    ...
    def _encode(value: Any) -> str:
        ...
        if isinstance(value, dict):
            return "{" + ",".join(
                f'"{k}":{_encode(value[k])}' for k in sorted(value)
            ) + "}"
        raise TypeError(f"Unsupported value type: {type(value)!r}")

    content = receipt.to_dict()
    json_body = "{" + ",".join(
        f'"{key}":{_encode(content[key])}' for key in sorted(content)
    ) + "}"
    return json_body + "\n"
