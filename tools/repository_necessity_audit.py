...
from typing import DefaultDict, Dict, Iterable, List, Mapping, Sequence, Set
...
def _run_pytest_collect(repo_root: Path) -> Sequence[str]:
    ...
    if completed.returncode not in {0, 5}:  # pytest returns 5 when tests missing
        raise AuditError(
...
def _classify_files(
    ...
    return AuditResult(used, test_only, redundant, unknown)
...
def _post_audit_receipt(result: AuditResult, repo_root: Path) -> Dict[str, object]:
    """Generate and persist the Tessrax audit receipt (DLK-001 gate 2)."""
...
def _post_audit_gate(result: AuditResult) -> Dict[str, object]:
    """Execute the second double-lock gate and return the receipt payload."""

    return _post_audit_receipt(result, REPO_ROOT)
