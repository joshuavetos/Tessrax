...
class ProvenanceService:
    ...
    def __init__(self, ledger_path: Optional[Path] = None) -> None:
        ...
        self._ledger_path = Path(configured) if configured else (ledger_path or Path("ledger/truth_api_ledger.jsonl"))
    ...
    def append_receipt(self, payload: Dict[str, any], *, seed: Optional[str] = None) -> ReceiptRecord:
    ...
    def verify_receipt(self, uuid: str) -> ReceiptRecord:
        ...
        for idx, entry in enumerate(entries):
            if entry["uuid"] == uuid:
                ...
                expected_hash = merkle_hash(payload=entry["payload"], prev_hash=prev_hash)
                if entry["merkle_hash"] != expected_hash:
                    raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Merkle hash mismatch")
                if not verify_signature(entry["payload"], entry["signature"]):
                    raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Signature mismatch")
                return ReceiptRecord(**entry)
